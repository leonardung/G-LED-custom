...
def autoregressive_test(
    ...
    for j, timesteps in tqdm(enumerate(all_results)):
        ...
        for direction_idx, direction in enumerate(["u", "v", "w"][:num_velocities]):
            ...
            ax.plot(
                range(len(error_evolution[direction])),
                error_evolution[direction],
                marker="o",
            )
            ...
            if len(avg_rmse_evolution[direction]) < len(error_evolution[direction]):
                avg_rmse_evolution[direction].extend(
                    [0]
                    * (
                        len(error_evolution[direction])
                        - len(avg_rmse_evolution[direction])
                    )
                )
    ...
    for direction in avg_rmse_evolution:
        avg_rmse_evolution[direction] = [
            rmse / len(all_results) for rmse in avg_rmse_evolution[direction]
        ]
        fig, ax = plt.subplots(figsize=(10, 6))
        ax.plot(
            range(len(avg_rmse_evolution[direction])),
            avg_rmse_evolution[direction],
            marker="o",
        )
