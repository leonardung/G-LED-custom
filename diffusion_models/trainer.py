...
from diffusion_models.imagen_pytorch import Imagen, NullUnet
from diffusion_models.elucidated_imagen_3d import ElucidatedImagen
from diffusion_models.data import cycle
    ...
    def inner(model, *args, **kwargs):
        ...
        if cast_device:
            all_args = tuple(
                map(
                    lambda t: (
                        t.to(device) if exists(t) and isinstance(t, torch.Tensor) else t
                    ),
                    all_args,
                )
            )
...
class ImagenTrainer(nn.Module):
    ...
    def wrap_unet(self, unet_number):
        ...
        self.unet_being_trained.to(self.mydevice)
