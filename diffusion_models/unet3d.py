...
from diffusion_models.t5 import t5_encode_text, get_encoded_dim, DEFAULT_T5_NAME
...
def resize_video_to(video, target_image_size, clamp_range=None):
    ...
    video = rearrange(video, "b c f d h w -> (b f) c d h w")
    ...
    out = rearrange(out, "(b f) c d h w -> b c f d h w", f=frames)
...
class PerceiverAttention(nn.Module):
    def __init__(self, *, dim, dim_head=64, heads=6, cosine_sim_attn=False):
        super().__init__()
...
class Conv4D(nn.Module):
    def __init__(
        ...
        """
        Combines 3D spatial convolutions (depth, height, width) with a causal temporal convolution.
        ...
        # 3D spatial convolution over (depth, height, width)
        ...
        # Initialize temporal convolution to be identity
        if exists(self.temporal_conv):
            nn.init.dirac_(self.temporal_conv.weight.data)  # Initialize to identity
            nn.init.zeros_(self.temporal_conv.bias.data)

    def forward(self, x, ignore_time=False):
        """
        Forward pass of Conv4D.

        Args:
            x: Input tensor of shape (batch, channels, frames, depth, height, width).
            ignore_time: If True, bypass the temporal convolution.
        Returns:
            Output tensor after spatial and temporal convolutions.
        """
        b, c, f, d, h, w = x.shape
        ...
        # Apply 3D spatial convolution
        if is_video:
            x = rearrange(x, "b c f d h w -> (b f) c d h w")

        x = self.spatial_conv(x)

        if is_video:
            x = rearrange(x, "(b f) c d h w -> b c f d h w", b=b)

        # Skip temporal convolution if ignored or not defined
        if ignore_time or not exists(self.temporal_conv):
            return x

        # Apply causal temporal convolution
        x = rearrange(x, "b c f d h w -> (b d h w) c f")
        ...
        x = rearrange(x, "(b d h w) c f -> b c f d h w", b=b, d=d, h=h, w=w)
...
class Conv4DManual(nn.Module):
    def __init__(self, dim_in, dim_out, kernel_size, stride=1, padding=0, **kwargs):
        """
        Simulates a 4D convolution (frames, depth, height, width) by applying Conv3D
        independently across the frames dimension.

        Args:
            dim_in: Number of input channels.
            dim_out: Number of output channels.
            kernel: Kernel size for the (depth, height, width) dimensions.
            stride: Stride size for the (depth, height, width) dimensions.
            padding: Padding size for the (depth, height, width) dimensions.
        """
        super(Conv4DManual, self).__init__()
    ...
    def forward(self, x):
        ...
        Args:
            x: Input tensor of shape (batch, channels, frames, depth, height, width).
        Returns:
            Output tensor of shape (batch, out_channels, frames, new_depth, new_height, new_width).
        """
        batch_size, channels, frames, depth, height, width = x.shape

        # Process each frame independently
        output_frames = []
        for f in range(frames):
            frame_output = self.conv3d(
                x[:, :, f]
            )  # Conv3D processes (batch, channels, depth, height, width)
            output_frames.append(frame_output)

        # Stack processed frames along the frames dimension
        output = torch.stack(output_frames, dim=2)  # Reconstruct frames dimension
        return output
...
def Upsample(dim, dim_out=None):
    ...
    return nn.Sequential(
        ...
        # Conv4DManual(dim, dim_out, 3, padding=1),
    )


class PixelShuffleUpsample(nn.Module):
    def __init__(self, dim, dim_out=None, upscale_factor=2):
        super().__init__()
        self.upscale_factor = upscale_factor
        dim_out = default(dim_out, dim)
        conv = Conv4DManual(dim, dim_out * (upscale_factor ** 3), kernel_size=1)
    ...
    def init_conv_(self, conv):
        ...
        conv_weight = torch.empty(o // (self.upscale_factor ** 3), i, f, h, w)
        nn.init.kaiming_uniform_(conv_weight)
        conv_weight = repeat(
            conv_weight, f"o ... -> (o {self.upscale_factor ** 3}) ..."
        )
        conv.weight.data.copy_(conv_weight)
        nn.init.zeros_(conv.bias.data)

    def forward(self, x):
        # Upscale channels and redistribute
        out = self.net(x)
        b, c, f, d, h, w = out.shape
        scale = self.upscale_factor

        # Reshape for 3D pixel shuffle
        out = rearrange(
            out,
            "b (c dscale hscale wscale) f d h w -> b c f (d dscale) (h hscale) (w wscale)",
            dscale=scale,
            hscale=scale,
            wscale=scale,
        )
...
def Downsample(dim, dim_out=None):
    ...
    return nn.Sequential(
        Rearrange(
            "b c f (d p0) (h p1) (w p2) -> b (c p0 p1 p2) f d h w", p0=2, p1=2, p2=2
        ),
        Conv4DManual(dim * 8, dim_out, kernel_size=1),
    )
...
class Block(nn.Module):
    def __init__(self, dim, dim_out, groups=8, norm=True):  # I changed it 8->6
        super().__init__()
        self.groupnorm = nn.GroupNorm(groups, dim) if norm else Identity()
        self.activation = nn.SiLU()
        self.project = Conv4D(dim, dim_out, 3, padding=1)
...
class ResnetBlock(nn.Module):
    def __init__(
        ...
        **attn_kwargs,
        ...
        self.res_conv = Conv4DManual(dim, dim_out, 1) if dim != dim_out else Identity()

    def forward(self, x, time_emb=None, cond=None, ignore_time=False):
        scale_shift = None
        if exists(self.time_mlp) and exists(time_emb):
            time_emb = self.time_mlp(time_emb)
            time_emb = rearrange(time_emb, "b c -> b c 1 1 1 1")
...
class LinearAttention(nn.Module):
    def __init__(
        ...
        self.to_q = nn.Sequential(
            nn.Dropout(dropout),
            Conv4DManual(dim, inner_dim, 1, bias=False),
            Conv4DManual(
                inner_dim, inner_dim, 3, bias=False, padding=1, groups=inner_dim
            ),
        )
        ...
        self.to_out = nn.Sequential(
            Conv4DManual(inner_dim, dim, 1, bias=False), ChanLayerNorm(dim)
        )
...
class GlobalContext(nn.Module):
    """Superior form of squeeze-excitation that is attention-esque."""

    def __init__(self, *, dim_in, dim_out):
        super().__init__()
        self.to_k = Conv4DManual(dim_in, 1, 1)
        hidden_dim = max(3, dim_out // 2)

        self.net = nn.Sequential(
            Conv4DManual(dim_in, hidden_dim, 1),
            nn.SiLU(),
            Conv4DManual(hidden_dim, dim_out, 1),
            nn.Sigmoid(),
        )

    def forward(self, x):
        b, c, f, d, h, w = x.shape
        context = self.to_k(x)  # context: (b, 1, f, d, h, w)

        # Flatten spatial dimensions
        x = rearrange(x, "b c f d h w -> b c (f d h w)")  # x: (b, c, n)
        context = rearrange(
            context, "b 1 f d h w -> b 1 (f d h w)"
        )  # context: (b, 1, n)

        # Apply softmax over the spatial dimension
        context = context.softmax(dim=-1)  # context: (b, 1, n)

        # Compute the global context vector
        out = torch.einsum("b i n, b c n -> b c i", context, x)  # out: (b, c, 1)

        # Reshape to match the expected input shape of self.net
        out = rearrange(out, "b c 1 -> b c 1 1 1 1")  # out: (b, c, 1, 1, 1, 1)
...
def ChanFeedForward(
    ...
    return nn.Sequential(
        ChanLayerNorm(dim),
        Conv4DManual(dim, hidden_dim, 1, bias=False),
        nn.GELU(),
        ChanLayerNorm(hidden_dim),
        Conv4DManual(hidden_dim, dim, 1, bias=False),
    )
...
class Unet3D(nn.Module):
    def __init__(
        ...
        *,
        ...
        temporal_peg = lambda dim: Residual(
            nn.Sequential(
                Pad(temporal_peg_padding), Conv4DManual(dim, dim, (3, 1, 1), groups=dim)
            )
        )
        ...
        temporal_attn = lambda dim: EinopsToAndFrom(
            ...
            Residual(
                Attention(
                    dim,
                    **{**attn_kwargs, "causal": time_causal_attn, "init_zero": True},
                )
            ),
        )
            ...
            if not memory_efficient:
                post_downsample = (
                    ...
                    if not is_last
                        ...
                        Conv4DManual(dim_in, dim_out, 3, padding=1),
                        Conv4DManual(dim_in, dim_out, 1),
                    )
                )
        ...
        self.mid_attn = (
            EinopsToAndFrom(
                "b c f h w d",
                "b (f h w d) c",
                Residual(Attention(mid_dim, **attn_kwargs)),
            )
            if attend_at_middle
            else None
        )
    ...
    def forward(
        ...
        *,
        ...
        assert (
            x.ndim == 6
        ), "input to 3d unet must have 6 dimensions (batch, channels, time, height, width, depth)"
