...
from diffusion_models.imagen_pytorch import (
...
from diffusion_models.unet3d import Unet3D, resize_video_to
...
class ElucidatedImagen(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.encode_text = partial(
            t5_encode_text, name=text_encoder_name, device=device
        )
        ...
        self.image_depth = cast_tuple(image_depth)
            ...
            for r in reversed(range(resample_times)):
                ...
                if physics_guidance is not None:
                    for phyguid in physics_guidance:
                        num_ite = (
                            args_sample.num_ite[0]
                            if not is_last_timestep
                            else args_sample.num_ite[1]
                        )
                        sigma_extra = (
                            args_sample.sigma_extra[0]
                            if not is_last_timestep
                            else args_sample.sigma_extra[1]
                        )
                        beta_guide = (
                            args_sample.beta_guide[0]
                            if not is_last_timestep
                            else args_sample.beta_guide[1]
                        )  # beta_guide = 0.0180 if not is_last_timestep else 0.0080
                        RZ = lambda Z: phyguid(
                        ...
                        for ite in range(num_ite):
                            ...
                            guide = (
                                ...
                                / dRdZ.norm()
                                * torch.max(
                                    torch.norm(added_noise),
                                    torch.norm(added_noise) * 0 + sigma_extra,
                                )
                                * beta_guide
                            )  #
                            ...
                            R = RZ(images_hat)
                            if (
                                is_last_resample_step
                                and is_last_timestep
                                and ite % 9 == 0
                            ):
                                print(
                                    "Residual "
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.is_video and images.ndim == 5:
        ...
        # return average loss
        # print(f"{losses.mean()=}")
        # z_size = images.shape[2]
        # middle_timesteps = list(range(0, z_size, 3))

        # # Create the plot
        # fig, axes = plt.subplots(
        #     len(middle_timesteps), 3, figsize=(10, 5 * len(middle_timesteps))
        # )

        # for i, timestep in enumerate(middle_timesteps):
        #     # Extract the slice at the timestep
        #     img_slice = images[0, 0, timestep, :, :, 8].cpu().detach().numpy()
        #     denoised_slice = (
        #         denoised_images[0, 0, timestep, :, :, 8].cpu().detach().numpy()
        #     )
        #     noised_slice = noised_images[0, 0, timestep, :, :, 8].cpu().detach().numpy()

        #     # Plot the original image
        #     axes[i, 0].imshow(img_slice, cmap="jet", extent=[0, 4, 0, 10])
        #     axes[i, 0].set_title(f"Image {timestep})")
        #     axes[i, 0].axis("off")

        #     # Plot the denoised image
        #     axes[i, 1].imshow(denoised_slice, cmap="jet", extent=[0, 4, 0, 10])
        #     axes[i, 1].set_title(f"Denoised {timestep})")
        #     axes[i, 1].axis("off")
            
        #     # Plot the denoised image
        #     axes[i, 2].imshow(noised_slice, cmap="jet", extent=[0, 4, 0, 10])
        #     axes[i, 2].set_title(f"Noised {timestep})")
        #     axes[i, 2].axis("off")
