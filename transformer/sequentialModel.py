...
class Attention(nn.Module):
    ...
    def __init__(self, nx, num_timesteps, config, scale=False):
        ...
        self.c_attn = Conv1D(
            nx * 3, nx
        )  # Kindly reminder: input_size = [..., nx] and output_size = [..., 3 * nx]
    ...
    def forward(
        ...
        query, key, value = x.split(
            self.split_size, dim=2
        )  # HanGao: wouldn't it be more general if dim=-1?
...
class SequentialModel(nn.Module):
    ...
    def forward(
        ...
        position_embeds[:, :, ::2] = torch.sin(
            ...
            ** (
                2
                * i.type(torch.FloatTensor).to(inputs_embeds.device)
                / self.config.n_embd
            )
        )
...
class SeqModelWithMLP(nn.Module):
    def __init__(self, config):  # in MLP: n_state=3072 (4 * n_embd)
        ...
        self.TokenModel = MLPDense(
            config.paraEnrichDim, config.n_embd, [200, 200], True
        )
